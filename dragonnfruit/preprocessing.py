...
def _extract_fragments(
    ...
    with gzip.open(filename, "r") as infile:
        for i, line in tqdm(
            ...
            if start > chrom_sizes[chrom]:
                raise ValueError(
                    "Read {}:{}-{} maps off chrom {}".format(chrom, start, end, chrom)
                )
    ...
    if include_cells is not None and ni != len(_included_cells):
        raise ValueError(
            "Not all cells in inclusion list have been"
            + " observed. Inclusion={}, Observed={}".format(ni, len(_included_cells))
        )
...
def extract_fragments(
    ...
    results = Parallel(n_jobs=n_jobs)(
        ...
        for i, filename, _include, _exclude, prefix in zip(
            range(n_files),
            fragments,
            _include_cells,
            _exclude_cells,
...
def extract_cellxpeak(X, peaks, chroms=None, verbose=False):
    ...
    if isinstance(peaks, str):
        ...
        peaks = pandas.read_csv(peaks, sep="\t", names=names, usecols=(0, 1, 2))
    ...
    peak_counts = scipy.sparse.csc_matrix(
        (data, indices, indptr), shape=(n, n_peaks), dtype="int8"
    )
    return peak_counts
...
def extract_cellxtile(X, tile_size, chroms=None, verbose=False):
    ...
    tile_counts = scipy.sparse.csc_matrix(
        (data, indices, indptr), shape=(n, n_tiles), dtype="int8"
    )
    return tile_counts


def preprocess_sparse_atac(
    X_cscs,
    peaks=None,
    tile_size=None,
    chroms=None,
    ...
    if peaks is not None:
        counts = extract_cellxpeak(X_cscs, peaks, chroms=chroms, verbose=verbose)
    elif peaks is None and tile_size is not None:
        counts = extract_cellxtile(X_cscs, tile_size, chroms=chroms, verbose=verbose)
    else:
