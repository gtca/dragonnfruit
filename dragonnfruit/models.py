...
class DragoNNFruit(torch.nn.Module):
    ...
    def __init__(self, bias, accessibility, name, verbose=True):
        ...
        self.logger = Logger(
            [
                ...
                "Training MNLL",
                "Validation MNLL",
                "Validation Profile Correlation",
                "Validation Count Correlation",
                "Saved?",
            ],
            verbose=verbose,
        )
    ...
    def forward(self, X, cell_states, read_depths):
        ...
        return read_depths + self.bias(X)[0][:, 0] + self.accessibility(X, cell_states)

    @torch.no_grad()
    def predict(
        self, X, cell_states, read_depths, batch_size=16, reduction=None, verbose=False
    ):
    ...
    @torch.no_grad()
    def predict_cross(
        self, X, cell_states, read_depths, batch_size=64, reduction=None, verbose=False
    ):
        ...
        for i in trange(n_states, disable=not verbose):
            _y_hat = self.predict(
                X,
                cell_states[i : i + 1].expand(X.shape[0], -1),
    ...
    def fit(
        ...
        for i, (X, y, cell_states, read_depths) in enumerate(training_data):
            ...
            if i % validation_iter == 0:
                ...
                y_hat = torch.nn.functional.log_softmax(
                    y_hat.flatten(), dim=-1
                ).reshape(*y_hat.shape)
                y_hat = torch.exp(y_hat)
                valid_profile_corr = pearson_corr(y_hat, y_valid).mean().item()
...
class DynamicBPNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fconv = torch.nn.Conv1d(
            n_filters, 1, kernel_size=75, padding=37, bias=conv_bias
        )

        self.biases = torch.nn.ModuleList(
            [torch.nn.Linear(controller.n_outputs, n_filters) for i in range(n_layers)]
        )
